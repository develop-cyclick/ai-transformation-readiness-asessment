          ...
          <div className="grid md:grid-cols-2 gap-6 mb-8">
            <div className="flex items-center space-x-4 p-4 bg-blue-50 rounded-lg">
              ...
              <div>
                ...
                <p className="text-2xl font-bold text-gray-900">{totalQuestions} ข้อ</p>
              </div>
            </div>
            ...
            <div className="flex items-center space-x-4 p-4 bg-purple-50 rounded-lg">
              ...
              <div>
                ...
                <p className="text-2xl font-bold text-gray-900">{totalSections} ส่วน</p>
              </div>
            </div>
          </div>

          <div className="space-y-4 mb-8">
            <h2 className="text-2xl font-bold text-gray-900 mb-4">หัวข้อในแบบประเมิน</h2>
            <div className="grid md:grid-cols-2 gap-3">
              {questionnaireData.sections.map((section) => (
                <div key={section.id} className="flex items-start space-x-3 p-3 bg-gray-50 rounded-lg hover:bg-gray-100 transition-colors">
                  <span className="flex-shrink-0 w-6 h-6 bg-blue-600 text-white rounded-full flex items-center justify-center text-sm font-bold">
                    {section.id}
                  </span>
                  <div className="flex-1 min-w-0">
                    <p className="text-sm font-medium text-gray-900">{section.title}</p>
                    <p className="text-xs text-gray-600">{section.questions.length} คำถาม</p>
                  </div>
                </div>
              ))}
            </div>
            ...
            <div className="flex">
              ...
              <div className="ml-3">
                <p className="text-sm text-yellow-700">
                  <strong>หมายเหตุ:</strong> คุณสามารถบันทึกและกลับมาทำต่อได้ทุกเมื่อ ไม่จำเป็นต้องทำให้เสร็จในครั้งเดียว
                </p>
              </div>
            </div>
          </div>
